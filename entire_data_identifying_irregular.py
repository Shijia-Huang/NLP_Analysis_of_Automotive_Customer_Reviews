    ...
    for entity in detected_list.loc[i, 'Entities']:
        ...
        for j in range(len(match_irregular1)):
            ...
            if (' ' in model) and (detected == model.split(' ', 1)[0] or detected == model.split(' ', 1)[1]
                                   or detected == model.replace(' ', '-') or detected == model.replace(' ',
                                                                                                       '_') or detected == model.replace(
                        ' ', '')):
                model_ID = model_id

                if (' ' not in brand) and ('-' not in brand) and brand in texts:
                    brand_ID = brand_id
                elif (' ' in brand) and ((brand.split(' ', 1)[0] in texts) or (brand.split(' ', 1)[1] in texts) or (
                        brand.replace(' ', '-') in texts)):
                    brand_ID = brand_id
                elif ('-' in brand) and ((brand.split('-', 1)[0] in texts) or (brand.split('-', 1)[1] in texts) or (
                        brand.replace('-', ' ') in texts)):
                    brand_ID = brand_id
